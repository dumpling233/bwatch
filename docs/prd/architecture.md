...
- 图表组件为 SVG 和占位提示预留固定高度：单主播走势使用小图高度，总览/历史使用聚合图高度；加载、空数据、错误和成功状态只替换图表内容。
