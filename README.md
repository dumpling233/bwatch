...
选择主播后，级联控件下方会显示固定高度的“场次峰值”折线图，按时间正序展示每场直播的最高在线人数。默认显示最近 30 场，可切换最近 10 场或全部；悬浮数据点可查看完整起止时间、峰值和时长，点击数据点与场次下拉框使用同一套历史定位逻辑。图表使用与其他走势图一致的 360px 最小宽度并随面板宽度自适应，不再增加二级横向滚动条；坐标标签保留独立安全边距，面板缩放不会拉伸文字或线条。
